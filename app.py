...
st.set_page_config(page_title="Passold Sistemas de Fachadas", layout="wide", initial_sidebar_state="collapsed")
...
st.markdown("""
    ...
    .topbar-logo-box img { width: 34px; height: 34px; object-fit: contain; }
    .topbar-brand {
    ...
    div.stButton > button[kind="primary"] { background-color: #EA580C; }
    div.stButton > button[kind="primary"]:hover { background-color: #C2410C; }
    ...
    /* ── BADGES ───────────────────────────────────────── */
    .badge-obra  { background:#FFF7ED; color:#C2410C; padding:3px 9px; border-radius:3px; font-weight:700; font-size:11px; letter-spacing:0.5px; text-transform:uppercase; }
    .badge-edt   { background:#EEF2FF; color:#3730A3; padding:3px 9px; border-radius:3px; font-weight:700; font-size:11px; letter-spacing:0.5px; }
    .badge-lote  { background:#F0FDF4; color:#15803D; padding:3px 9px; border-radius:3px; font-weight:700; font-size:11px; letter-spacing:0.5px; }
    .badge-status-ok      { background:#DCFCE7; color:#166534; padding:3px 9px; border-radius:3px; font-weight:600; font-size:11px; }
    .badge-status-warn    { background:#FEF9C3; color:#854D0E; padding:3px 9px; border-radius:3px; font-weight:600; font-size:11px; }
    .badge-status-danger  { background:#FEE2E2; color:#991B1B; padding:3px 9px; border-radius:3px; font-weight:600; font-size:11px; }
    ...
    /* ── STATUS BARS ──────────────────────────────────── */
    .bar-ok      { border-left:4px solid #22C55E; background:#F0FDF4; padding:12px 16px; border-radius:0 6px 6px 0; margin-bottom:10px; }
    .bar-warn    { border-left:4px solid #EAB308; background:#FEFCE8; padding:12px 16px; border-radius:0 6px 6px 0; margin-bottom:10px; }
    .bar-danger  { border-left:4px solid #EF4444; background:#FFF5F5; padding:12px 16px; border-radius:0 6px 6px 0; margin-bottom:10px; }
    .bar-neutral { border-left:4px solid #94A3B8; background:#F8FAFC; padding:12px 16px; border-radius:0 6px 6px 0; margin-bottom:10px; }
    .bar-active  { border-left:4px solid #1E3A8A; background:#EFF6FF; padding:12px 16px; border-radius:0 6px 6px 0; margin-bottom:10px; }
    ...
    /* ── STATUS ENGENHARIA ────────────────────────────── */
    .eng-badge-0 { background:#FEE2E2; color:#991B1B; padding:3px 10px; border-radius:3px; font-size:11px; font-weight:700; letter-spacing:0.5px; }
    .eng-badge-1 { background:#FEF9C3; color:#854D0E; padding:3px 10px; border-radius:3px; font-size:11px; font-weight:700; letter-spacing:0.5px; }
    .eng-badge-2 { background:#DBEAFE; color:#1E40AF; padding:3px 10px; border-radius:3px; font-size:11px; font-weight:700; letter-spacing:0.5px; }
    .eng-badge-3 { background:#DCFCE7; color:#166534; padding:3px 10px; border-radius:3px; font-size:11px; font-weight:700; letter-spacing:0.5px; }
    .eng-badge-4 { background:#F1F5F9; color:#475569; padding:3px 10px; border-radius:3px; font-size:11px; font-weight:700; letter-spacing:0.5px; }
    ...
    .login-wrapper {
        ...
        background: linear-gradient(135deg, #0F1F3D 0%, #1E3A8A 60%, #2563EB 100%);
    }
    .login-card {
        background: #FFFFFF;
        border-radius: 8px;
    ...
    /* ── DATAFRAMES ───────────────────────────────────── */
    .stDataFrame { border: 1px solid #E2E8F0; border-radius: 6px; overflow: hidden; }
...
# ── LOGO HELPER ────────────────────────────────────────────
def get_logo_base64():
    """Tenta carregar logo de ./logo.png ou retorna None."""
    for path in ["logo.png", "logo.jpg", "logo.jpeg", "assets/logo.png"]:
        if os.path.exists(path):
            with open(path, "rb") as f:
                return base64.b64encode(f.read()).decode()
    return None

def render_topbar(nome_usuario, setor_usuario):
    logo_b64 = get_logo_base64()
    if logo_b64:
        logo_html = f'<div class="topbar-logo-box"><img src="data:image/png;base64,{logo_b64}" alt="Logo"/></div>'
    else:
        logo_html = '<div class="topbar-logo-box" style="background:rgba(255,255,255,0.18);"><span style="color:white;font-family:Barlow Condensed,sans-serif;font-weight:800;font-size:18px;">P</span></div>'
    ...
    st.markdown(f"""
        <div class="topbar">
            <div class="topbar-left">
                {logo_html}
                <div>
                    <div class="topbar-brand">PASSOLD</div>
                    <div class="topbar-sub">Sistemas de Fachadas</div>
                </div>
            </div>
            <div class="topbar-right">
                <div class="topbar-user">
                    <div class="topbar-user-name">{nome_usuario}</div>
                    <div class="topbar-user-role">{setor_usuario}</div>
                </div>
...
def inicializar_banco_de_dados():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS cronograma_macro (
            id SERIAL PRIMARY KEY, Obra TEXT, EDT TEXT UNIQUE,
            Tipo_Escopo TEXT, Etapa_Macro TEXT, Subdivisao TEXT, Tarefa TEXT,
            M2_Total_Tarefa REAL, Inicio_Previsto DATE, Termino_Obra DATE,
            Status TEXT DEFAULT 'Pendente',
            Status_Engenharia TEXT DEFAULT 'Aguardando Medicao In Loco',
            Prazo_Engenharia DATE, Data_Limite_Despacho DATE, Primeiro_Dia_Producao DATE)
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS itens_detalhado (
            id SERIAL PRIMARY KEY, Obra_Vinculada TEXT, EDT_Vinculado TEXT,
            Cod_Lote TEXT, Num_OP TEXT, Tipo_Material TEXT, Qtd_Caixas INTEGER,
            M2_Item REAL, Data_Producao_Programada DATE, Data_Limite_Obra DATE,
            Data_Despacho DATE, Romaneio_Chapas TEXT,
            Status_Item TEXT DEFAULT 'Pendente', Dificuldade INTEGER DEFAULT 3,
            Fase_Produtiva TEXT, Enviado_Logistica INTEGER DEFAULT 0)
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS usuarios (
            id SERIAL PRIMARY KEY, usuario TEXT UNIQUE,
            nome TEXT, setor TEXT, senha TEXT)
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS solicitacoes_prazo (
            id SERIAL PRIMARY KEY, edt TEXT, tarefa TEXT, prazo_atual TEXT,
            prazo_solicitado TEXT, justificativa TEXT, criado_por TEXT,
            status TEXT DEFAULT 'Pendente de Aprovacao', criado_em TEXT)
    """)
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS logistica_envios (
            id SERIAL PRIMARY KEY, item_id INTEGER, Obra_Vinculada TEXT,
            EDT_Vinculado TEXT, Cod_Lote TEXT, Num_OP TEXT, Tipo_Material TEXT,
            Qtd_Caixas INTEGER, M2_Item REAL, Romaneio_Chapas TEXT,
            Data_Limite_Despacho DATE, Data_Envio_Agendado DATE,
            Transportadora TEXT, Veiculo TEXT, Observacoes TEXT,
...
# ── DATAS ──────────────────────────────────────────────────
def subtrair_dias_uteis(data_base, n):
    data = data_base; contados = 0
    while contados < n:
        data -= timedelta(days=1)
        if data.weekday() < 5: contados += 1
    return data

def gerar_lote_unico(data_limite_obra, dias_logistica, dias_uteis_fab,
                     total_cx, total_m2, obra, edt, cod_lote, especificacao, txt_pav, dificuldade):
    dt_limite   = datetime.combine(data_limite_obra, datetime.min.time()) if not isinstance(data_limite_obra, datetime) else data_limite_obra
    dt_despacho = dt_limite - timedelta(days=int(dias_logistica))
    dt_inicio   = subtrair_dias_uteis(dt_despacho, int(dias_uteis_fab))
    return [{"Obra_Vinculada":obra,"EDT_Vinculado":edt,"Cod_Lote":cod_lote,"Num_OP":None,
             "Tipo_Material":especificacao,"Qtd_Caixas":int(total_cx),"M2_Item":float(round(total_m2,2)),
             ...
             "Romaneio_Chapas":txt_pav,"Status_Item":"Pendente","Dificuldade":int(dificuldade),
             "Fase_Produtiva":f"CORTE→MONTAGEM ({dias_uteis_fab} dias uteis)","Enviado_Logistica":0}]
...
def carregar_macro():
    conn = conectar_banco()
    df = pd.read_sql_query("SELECT * FROM cronograma_macro ORDER BY id", conn); conn.close()
    ...
    df = df.rename(columns={'Edt':'EDT','M2_Total_Tarefa':'M2_Total_Tarefa','Status_Engenharia':'Status_Engenharia',
                             'Tipo_Escopo':'Tipo_Escopo','Etapa_Macro':'Etapa_Macro'})
    return df

def carregar_micro():
    conn = conectar_banco()
    df = pd.read_sql_query("SELECT * FROM itens_detalhado ORDER BY Data_Producao_Programada ASC", conn); conn.close()
    for col in ['data_producao_programada','data_limite_obra']:
        if col in df.columns: df[col] = pd.to_datetime(df[col], errors='coerce')
    df.columns = [c.replace('_',' ').title().replace(' ','_') if c != 'id' else c for c in df.columns]
    df = df.rename(columns={'Edt_Vinculado':'EDT_Vinculado','Num_Op':'Num_OP'})
    return df

def carregar_fila_logistica():
    conn = conectar_banco()
    df = pd.read_sql_query("SELECT * FROM logistica_envios ORDER BY data_limite_despacho ASC NULLS LAST", conn); conn.close()
    for col in ['data_limite_despacho','data_envio_agendado']:
        if col in df.columns: df[col] = pd.to_datetime(df[col], errors='coerce')
    df.columns = [c.replace('_',' ').title().replace(' ','_') if c != 'id' else c for c in df.columns]
    df = df.rename(columns={'Edt_Vinculado':'EDT_Vinculado','Num_Op':'Num_OP','Item_Id':'item_id'})
    return df

def carregar_solicitacoes():
    conn = conectar_banco()
    df = pd.read_sql_query("SELECT * FROM solicitacoes_prazo ORDER BY id DESC", conn); conn.close()
    return df

def salvar_lotes_micro(lotes):
    if not lotes: return
    conn = conectar_banco(); cursor = conn.cursor()
    for l in lotes:
        cursor.execute("""INSERT INTO itens_detalhado
            (Obra_Vinculada,EDT_Vinculado,Cod_Lote,Num_OP,Tipo_Material,Qtd_Caixas,M2_Item,
             Data_Producao_Programada,Data_Limite_Obra,Romaneio_Chapas,Status_Item,Dificuldade,Fase_Produtiva,Enviado_Logistica)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
            (l['Obra_Vinculada'],l['EDT_Vinculado'],l['Cod_Lote'],l['Num_OP'],l['Tipo_Material'],
             l['Qtd_Caixas'],l['M2_Item'],l['Data_Producao_Programada'],l['Data_Limite_Obra'],
             l['Romaneio_Chapas'],l['Status_Item'],l['Dificuldade'],l['Fase_Produtiva'],l['Enviado_Logistica']))
    conn.commit(); conn.close()
...
def atualizar_cronograma_macro_datas(edt, prazo_eng, primeiro_prod, despacho):
    ...
    if row:
        ...
        novo_prazo=pd.to_datetime(prazo_eng); novo_primeiro=pd.to_datetime(primeiro_prod); novo_despacho=pd.to_datetime(despacho)
    cursor.execute("UPDATE cronograma_macro SET Prazo_Engenharia=%s,Primeiro_Dia_Producao=%s,Data_Limite_Despacho=%s WHERE EDT=%s",
...
def salvar_solicitacao(edt, tarefa, prazo_atual, prazo_sol, justif, criado_por):
    conn = conectar_banco(); cursor = conn.cursor()
    cursor.execute("""INSERT INTO solicitacoes_prazo (edt,tarefa,prazo_atual,prazo_solicitado,justificativa,criado_por,status,criado_em)
...
def enviar_para_logistica(row, limite_despacho):
    ...
    cursor.execute("""INSERT INTO logistica_envios
        (item_id,Obra_Vinculada,EDT_Vinculado,Cod_Lote,Num_OP,Tipo_Material,Qtd_Caixas,M2_Item,Romaneio_Chapas,Data_Limite_Despacho,Status_Logistica)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'Aguardando Agendamento')""",
        (int(row['id']),row['Obra_Vinculada'],row['EDT_Vinculado'],row['Cod_Lote'],row.get('Num_OP') or '',
         row['Tipo_Material'],int(row['Qtd_Caixas']),float(row['M2_Item']),row['Romaneio_Chapas'],
...
def agendar_envio(log_id, data_envio, transportadora, veiculo, obs, usuario):
    conn = conectar_banco(); cursor = conn.cursor()
    cursor.execute("""UPDATE logistica_envios SET Data_Envio_Agendado=%s,Transportadora=%s,Veiculo=%s,
        Observacoes=%s,Status_Logistica='Envio Agendado' WHERE id=%s""",
        (data_envio.strftime('%Y-%m-%d') if data_envio else None,transportadora,veiculo,obs,log_id))
    conn.commit(); conn.close()

def confirmar_despacho(log_id, usuario):
    conn = conectar_banco(); cursor = conn.cursor()
    cursor.execute("""UPDATE logistica_envios SET Status_Logistica='Despachado',Confirmado_Por=%s,Confirmado_Em=%s WHERE id=%s""",
...
def resetar_banco_dados_completo():
    conn = conectar_banco(); cursor = conn.cursor()
    for t in ['cronograma_macro','itens_detalhado','solicitacoes_prazo','logistica_envios']:
...
if not st.session_state.autenticado:
    # Fundo azul full-page via CSS extra
    st.markdown("""
        <style>
        .main { background: linear-gradient(135deg, #0F1F3D 0%, #1E3A8A 60%, #2563EB 100%) !important; min-height:100vh; }
        .main .block-container { padding-top: 8vh !important; }
        </style>
    """, unsafe_allow_html=True)
    ...
    logo_html = ""
    if logo_b64:
        logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="height:56px;margin-bottom:12px;"/><br/>'

    st.markdown(f"""
        <div style="max-width:380px;margin:0 auto;background:#fff;border-radius:8px;
            ...
            <div style="text-align:center;margin-bottom:28px;">
                ...
                <div style="font-family:'Barlow Condensed',sans-serif;font-weight:800;
                            font-size:30px;color:#0F1F3D;letter-spacing:0.5px;">PASSOLD</div>
                <div style="font-size:11px;color:#94A3B8;letter-spacing:2px;text-transform:uppercase;
                            margin-top:2px;">PCP & Controle Operacional</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
# Botão sair pequeno fora da topbar
with st.container():
    _, col_sair = st.columns([11,1])
    with col_sair:
        if st.button("Sair"):
...
abas_disponiveis = []
if setor in ["Master","Producao","Diretoria","Engenharia"]: abas_disponiveis.append("Producao")
if setor in ["Master"]:                                     abas_disponiveis.append("Liberar OPs")
if setor in ["Master","Diretoria"]:                         abas_disponiveis.append("Visao Macro")
if setor in ["Master"]:
    abas_disponiveis.append("Lotes")
    abas_disponiveis.append("Nova Obra")
if setor in ["Master","Engenharia"]:                        abas_disponiveis.append("Engenharia")
if setor in ["Master","Logistica"]:                         abas_disponiveis.append("Logistica")
if setor in ["Master"]:                                     abas_disponiveis.append("Configuracoes")
...
ENG_BADGE_CLASS = ["eng-badge-0","eng-badge-1","eng-badge-2","eng-badge-3","eng-badge-4"]

def eng_badge(status):
    idx = ESTADOS_ENG.index(status) if status in ESTADOS_ENG else 0
    return f'<span class="{ENG_BADGE_CLASS[idx]}">{status}</span>'
...
for nome_aba, aba_objeto in zip(abas_disponiveis, abas_objetos):
    ...
    if nome_aba == "Producao":
        ...
        with aba_objeto:
            section_header("Mural de Producao", f"Hoje — {HOJE_PROJETO.strftime('%A, %d de %B de %Y')}")

            obras_tv = ["Todas as obras"] + (list(df_banco_micro['Obra_Vinculada'].dropna().unique()) if not df_banco_micro.empty else [])
            obra_tv  = st.selectbox("Filtrar:", obras_tv, key="sb_obra_tv")
            ...
            if not df_banco_micro.empty:
                ...
                if not df_base.empty:
                    ...
                    for _, row in df_base.iterrows():
                        ...
                        ini_ultima = (pd.to_datetime(dt_fim) - timedelta(days=6)).date()
                        ini_ultima = max(ini_ultima, dt_ini)
                        dia = dt_ini
                        while dia <= dt_fim:
                            r = row.to_dict(); r['_dia'] = dia
                            r['_pode_concluir'] = (dia >= ini_ultima)
                    ...
                    with c1:
                        if st.button("← Anterior", use_container_width=True, key="btn_ant"):
                            ...
                            if st.session_state.prog_mes == 0: st.session_state.prog_mes=12; st.session_state.prog_ano-=1
                            st.rerun()
                    with c2:
                        st.markdown(f'<div class="cal-month-title">{nomes_meses[st.session_state.prog_mes].upper()} {st.session_state.prog_ano}</div>', unsafe_allow_html=True)
                    with c3:
                        if st.button("Próximo →", use_container_width=True, key="btn_prox"):
                            st.session_state.prog_mes += 1
                            if st.session_state.prog_mes == 13: st.session_state.prog_mes=1; st.session_state.prog_ano+=1
                            st.rerun()
                    ...
                    for i, nome in enumerate(["DOM","SEG","TER","QUA","QUI","SEX","SAB"]):
                        cols_h[i].markdown(f'<div class="cal-header-day">{nome}</div>', unsafe_allow_html=True)
                        ...
                        for i, data_dia in enumerate(semana):
                            with cols[i]:
                                if data_dia.month == st.session_state.prog_mes:
                                    ...
                                    tem_urgente = lotes_dia['_pode_concluir'].any() if not lotes_dia.empty else False

                                    if n_lotes > 0:
                                        obras_d = lotes_dia['Obra_Vinculada'].unique()
                                        label_o = obras_d[0] if len(obras_d)==1 else f"{len(obras_d)} obras"
                                        dot = "🟠" if tem_urgente else "🔵"
                                        btn_label = f"{data_dia.day}  {dot}\n{n_lotes} lote(s)"
                                        if st.button(btn_label, key=f"btn_{data_dia}", use_container_width=True):
                                            st.session_state.dia_clicado_tv = data_dia
                                    else:
                                        num_css = "cal-cell-today-num" if eh_hoje else "cal-cell-day-num"
                                        border  = "border:2px solid #BFDBFE;" if eh_hoje else ""
                                        st.markdown(f"""
                                            <div class="cal-cell-empty" style="{border}">
                                                <span class="{num_css}">{data_dia.day}</span>
                                            </div>""", unsafe_allow_html=True)
                                else:
                                    st.markdown('<div style="height:76px;"></div>', unsafe_allow_html=True)

                    st.markdown("<hr style='border:none;border-top:2px solid #E2E8F0;margin:20px 0 16px 0;'/>", unsafe_allow_html=True)
                    ...
                    st.markdown(f"""
                        <div style="display:flex;align-items:baseline;gap:12px;margin-bottom:16px;">
                            <span style="font-family:'Barlow Condensed',sans-serif;font-weight:800;font-size:24px;color:#0F1F3D;">
                                {dia_sel.strftime('%d/%m/%Y')}
                            </span>
                            <span style="font-size:13px;color:#64748B;text-transform:uppercase;letter-spacing:1px;font-weight:600;">
                                Lotes em producao
                            </span>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    if lotes_sel.empty:
                        st.markdown('<div class="bar-neutral"><span style="color:#64748B;font-size:14px;">Clique em um dia com lotes no calendário acima.</span></div>', unsafe_allow_html=True)
                    else:
                        kc1,kc2,kc3 = st.columns(3)
                        kc1.metric("Lotes", lotes_sel['Cod_Lote'].nunique())
                        kc2.metric("Caixas", int(lotes_sel['Qtd_Caixas'].sum()))
                        kc3.metric("Metragem", f"{lotes_sel['M2_Item'].sum():.1f} m²")
                        ...
                        for _, row in lotes_sel.iterrows():
                            ...
                            card_class = "lote-card-final" if pode_concluir else "lote-card-normal"

                            col_info, col_prazo, col_acao = st.columns([5, 2, 2])

                            with col_info:
                                tag_semana = '<span class="ultima-semana-tag">Ultima semana — liberar para concluir</span>' if pode_concluir else f'<span class="em-producao-tag">Em producao</span>'
                                op_txt = row['Num_OP'] if row.get('Num_OP') else "Aguardando OP"
                                st.markdown(f"""
                                    <div class="{card_class}">
                                        <div>
                                            <span class="badge-obra">{row['Obra_Vinculada']}</span>&nbsp;
                                            <span class="badge-edt">{row['EDT_Vinculado']}</span>&nbsp;
                                            <span class="badge-lote">{row['Cod_Lote']}</span>
                                        </div>
                                        <div class="lote-title">{row['Tipo_Material']}</div>
                                        <div class="lote-meta">{int(row['Qtd_Caixas'])} caixas &nbsp;·&nbsp; {row['M2_Item']:.2f} m² &nbsp;·&nbsp; {row['Romaneio_Chapas']}</div>
                                        <div class="lote-meta">OP: {op_txt} &nbsp;·&nbsp; {row.get('Fase_Produtiva','—')}</div>
                                        {tag_semana}
                                    </div>
                                """, unsafe_allow_html=True)

                            with col_prazo:
                                dias_rest = (pd.to_datetime(row['Data_Limite_Obra']).date() - dia_sel).days
                                cor_dias  = "#EA580C" if dias_rest <= 7 else "#0F1F3D"
                                st.markdown(f"""
                                    <div style="background:#FFFFFF;border:1px solid #E2E8F0;border-radius:6px;
                                                padding:14px 16px;text-align:center;">
                                        <div class="prazo-label">Prazo na obra</div>
                                        <div class="prazo-valor">{dt_f}</div>
                                        <div style="font-family:'Barlow Condensed',sans-serif;font-weight:700;
                                                    font-size:28px;color:{cor_dias};margin-top:4px;">{dias_rest}d</div>
                                        <div style="font-size:10px;color:#94A3B8;letter-spacing:1px;">RESTANTES</div>
                                    </div>
                                """, unsafe_allow_html=True)
                            ...
                            with col_acao:
                                ...
                                if pode_concluir:
                                    if st.button("CONCLUIR LOTE", key=f"baixa_{row['id']}", type="primary", use_container_width=True):
                                        limite_desp = None
                                        if not df_banco_macro.empty:
                                            fr = df_banco_macro[df_banco_macro['EDT'] == row['EDT_Vinculado']]
                                            if not fr.empty: limite_desp = fr.iloc[0].get('Data_Limite_Despacho')
                                        conn = conectar_banco(); cursor = conn.cursor()
                                    ...
                                    st.markdown("""
                                        <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:6px;
                                            ...
                                            <div style="font-size:11px;color:#94A3B8;font-weight:600;
                                                ...
                                                Disponivel na<br/>ultima semana
                                            </div>
                                        </div>
                                    """, unsafe_allow_html=True)
                else:
                    st.markdown('<div class="bar-ok"><span style="color:#166534;font-weight:600;">Sem lotes liberados para este filtro.</span></div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="bar-neutral"><span style="color:#475569;">Nenhum lote liberado no sistema ainda.</span></div>', unsafe_allow_html=True)

    # ════════════════════════════════════════════════════
    # LIBERAR OPS
    # ════════════════════════════════════════════════════
    elif nome_aba == "Liberar OPs":
        with aba_objeto:
            section_header("Liberar Ordens de Producao", "Selecione os lotes e defina o prefixo da OP")
            if obra_selecionada and not df_banco_micro.empty:
                df_pend = df_banco_micro[(df_banco_micro['Obra_Vinculada']==obra_selecionada)&(df_banco_micro['Status_Item']=="Pendente")].copy()
                if not df_pend.empty:
                    df_pend['Selecionar'] = False
                    cols_exib = [c for c in ['id','Cod_Lote','Tipo_Material','Qtd_Caixas','M2_Item','Fase_Produtiva','Data_Producao_Programada','Romaneio_Chapas','Selecionar'] if c in df_pend.columns]
                    ...
                    if st.button("Liberar para producao"):
                        if ids_sel:
                            ...
                            for item_id in ids_sel:
                                cursor.execute("UPDATE itens_detalhado SET Status_Item='Liberado para Fabrica',Num_OP=%s WHERE id=%s",
                                               (f"{prefixo}{str(item_id).zfill(3)}",item_id))
                            conn.commit(); conn.close()
                            st.toast("OPs liberadas!"); time.sleep(0.5); st.rerun()
                        else: st.warning("Selecione pelo menos um item.")
                else: st.success("Todos os lotes ja foram liberados.")
            else: st.info("Nenhum lote pendente encontrado.")
    ...
    # ════════════════════════════════════════════════════
        ...
        with aba_objeto:
            section_header("Dashboard Executivo", "Visao consolidada da producao")
            df_dir = df_banco_micro[df_banco_micro['Obra_Vinculada']==obra_selecionada].copy() if obra_selecionada and not df_banco_micro.empty else df_banco_micro.copy()
            if not df_dir.empty:
                data_max = df_dir['Data_Limite_Obra'].max()
                c1,c2,c3 = st.columns(3)
                c1.metric("Metragem Total",       f"{df_dir['M2_Item'].sum():,.1f} m²")
                c2.metric("Frentes de Trabalho",  f"{df_dir['EDT_Vinculado'].nunique()}")
                c3.metric("Prazo Mais Distante",   data_max.strftime('%d/%m/%Y') if prazo_valido(data_max) else "N/A")
                st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
                st.markdown('<div style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;margin-bottom:8px;letter-spacing:0.5px;">CARGA SEMANAL</div>', unsafe_allow_html=True)
                ...
                if not df_lib.empty:
                    ...
                    def fmt_sem(r):
                        try:
                            ...
                            return f"Sem {int(r['Num_Semana']):02d}  ({s.strftime('%d/%m')} – {(s+timedelta(days=6)).strftime('%d/%m/%Y')})"
                    ...
                    res = df_lib.groupby(['Ano_Semana','Num_Semana','Periodo','Obra_Vinculada']).agg(
                        ...
                        Evolucao=('Status_Item',lambda x:f"{(x=='Concluido').sum()/len(x)*100:.0f}%")
                ...
                else: st.warning("Nenhuma OP liberada ainda.")
                st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
                st.markdown('<div style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;margin-bottom:8px;letter-spacing:0.5px;">GANTT — OCUPACAO DA FABRICA</div>', unsafe_allow_html=True)
                ...
                if not df_gantt.empty:
                    fig = px.timeline(df_gantt,x_start="Inicio",x_end="Fim",y="EDT_Vinculado",
                                      ...
                                      color_discrete_sequence=["#1E3A8A","#EA580C","#0891B2","#15803D","#7C3AED"])
    ...
    # ════════════════════════════════════════════════════
        ...
        with aba_objeto:
            section_header("Fatiamento de Lotes", "Defina as entregas e o sistema calcula o cronograma reverso")
            ...
            if obra_selecionada and not df_macro_filtrado.empty:
                ...
                with st.form("form_fatiamento"):
                    ...
                    with c1:
                        ...
                        row_sel = mapa_rows[edt_sel]; edt_puro=edt_sel.split(" - ")[0].strip()
                        cod_lote= st.text_input("Nome do Lote:")
                    ...
                    with c2:
                        ...
                        default_dt = pd.to_datetime(inicio_prev).date() if prazo_valido(inicio_prev) else (datetime.now()+timedelta(days=30)).date()
                        ...
                        st.markdown(f"""
                            <div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:6px;padding:14px 16px;">
                                <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#3B82F6;margin-bottom:8px;">Cronograma calculado</div>
                                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;">
                                    <div>
                                        <div style="font-size:10px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Inicio producao</div>
                                        <div style="font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;">{dt_inicio.strftime('%d/%m/%Y')}</div>
                                    </div>
                                    <div>
                                        <div style="font-size:10px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Saida da fabrica</div>
                                        <div style="font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;">{dt_desp.strftime('%d/%m/%Y')}</div>
                                    </div>
                                    <div>
                                        <div style="font-size:10px;color:#64748B;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Chegada na obra</div>
                                        <div style="font-family:'Barlow Condensed',sans-serif;font-weight:700;font-size:18px;color:#EA580C;">{data_alvo.strftime('%d/%m/%Y')}</div>
                                    </div>
                                </div>
                            </div>
                        """,unsafe_allow_html=True)
                    if "Liberados" not in str(row_sel.get('Status_Engenharia','')):
                        st.warning(f"Engenharia: `{row_sel.get('Status_Engenharia','—')}`")
                    if st.form_submit_button("Gerar Lote"):
                        if not cod_lote.strip(): st.error("Digite o nome do lote.")
                        else:
                            deletar_lotes_por_edt_lote(obra_selecionada,edt_puro,cod_lote.strip())
                            lote=gerar_lote_unico(data_alvo,int(dias_log),int(dias_fab),int(total_cx),float(total_m2),obra_selecionada,edt_puro,cod_lote.strip(),espec,txt_pav,dific)
                ...
                st.markdown('<div style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;margin-bottom:8px;">LOTES GERADOS</div>',unsafe_allow_html=True)
                ...
                if not df_ed_raw.empty:
                    ...
                    if not df_obra.empty:
                        ...
                        df_str=df_obra.copy()
                        df_edit=st.data_editor(df_str,key="editor_lotes",hide_index=True,use_container_width=True,disabled=["id","Obra_Vinculada","Num_OP"])
                        alteradas=[df_edit.loc[i] for i in df_edit.index if not df_edit.loc[i].equals(df_str.loc[i])]
                        if alteradas:
                            conn=conectar_banco(); cursor=conn.cursor()
                            for row in alteradas:
                                cursor.execute("""UPDATE itens_detalhado SET Cod_Lote=%s,Tipo_Material=%s,Qtd_Caixas=%s,M2_Item=%s,
                                    Data_Producao_Programada=%s,Data_Limite_Obra=%s,Romaneio_Chapas=%s,Status_Item=%s,Dificuldade=%s,Fase_Produtiva=%s WHERE id=%s""",
                                    (row['Cod_Lote'],row['Tipo_Material'],int(row['Qtd_Caixas']),float(row['M2_Item']),
                                     row['Data_Producao_Programada'],row['Data_Limite_Obra'],row['Romaneio_Chapas'],
                                     row['Status_Item'],int(row['Dificuldade']),row['Fase_Produtiva'],int(row['id'])))
                            conn.commit(); conn.close()
                            st.toast("Salvo!"); time.sleep(0.3); st.rerun()
                        st.markdown("**Remover lote:**")
                        lote_del=st.selectbox("",df_obra['Cod_Lote'].unique().tolist())
                        if st.button(f"Excluir {lote_del}"):
                            deletar_lotes_por_edt_lote(obra_selecionada,None,lote_del)
                            st.toast(f"{lote_del} removido!"); time.sleep(0.5); st.rerun()
                    else: st.info("Nenhum lote fatiado ainda.")

    # ════════════════════════════════════════════════════
    # NOVA OBRA
    # ════════════════════════════════════════════════════
    elif nome_aba == "Nova Obra":
        with aba_objeto:
            section_header("Cadastrar Nova Obra", "Registre frentes e defina o escopo de cada EDT")
            for k,v in [('mem_obra',''),('mem_frente',''),('mem_tarefa',''),
                        ('mem_dt_ini',datetime.now().date()),('mem_dt_fim',(datetime.now()+timedelta(days=90)).date())]:
            ...
            with st.form("form_obra"):
                ...
                with cd1: dt_ini=st.date_input("Inicio da instalacao:",value=st.session_state.mem_dt_ini,format="DD/MM/YYYY")
                with cd2: dt_fim=st.date_input("Prazo maximo:",value=st.session_state.mem_dt_fim,format="DD/MM/YYYY")
                if st.form_submit_button("Registrar frente"):
                    if not all([nome_obra.strip(),edt_cod.strip(),tarefa.strip(),subdiv.strip()]):
                        st.error("Preencha todos os campos.")
                    else:
                        st.session_state.mem_obra=nome_obra; st.session_state.mem_frente=frente
                        st.session_state.mem_tarefa=tarefa; st.session_state.mem_dt_ini=dt_ini; st.session_state.mem_dt_fim=dt_fim
                        conn=conectar_banco(); cursor=conn.cursor()
                        try:
                            cursor.execute("""INSERT INTO cronograma_macro
                                (Obra,EDT,Tipo_Escopo,Etapa_Macro,Subdivisao,Tarefa,M2_Total_Tarefa,Inicio_Previsto,Termino_Obra,Status,Status_Engenharia)
                                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,'Pendente','Aguardando Medicao In Loco')""",
                                (nome_obra,edt_cod,escopo,frente,subdiv,tarefa,float(m2_tot),dt_ini.strftime('%Y-%m-%d'),dt_fim.strftime('%Y-%m-%d')))
                            conn.commit(); st.toast("Frente registrada!"); time.sleep(0.4); st.rerun()
                        except Exception as e: st.error(f"EDT '{edt_cod}' ja existe ou erro: {e}")
                        finally: conn.close()
            if not df_banco_macro.empty:
                st.markdown("<hr style='border:none;border-top:2px solid #E2E8F0;margin:20px 0;'/>",unsafe_allow_html=True)
                st.markdown('<div style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;margin-bottom:8px;">FRENTES CADASTRADAS</div>',unsafe_allow_html=True)
                df_show=df_banco_macro.copy()
                for col in ['Inicio_Previsto','Termino_Obra']:
                    if col in df_show.columns: df_show[col]=pd.to_datetime(df_show[col],errors='coerce').dt.strftime('%d/%m/%Y')
                cols_s=[c for c in ['Obra','EDT','Subdivisao','Tarefa','M2_Total_Tarefa','Inicio_Previsto','Termino_Obra','Status_Engenharia'] if c in df_show.columns]
                st.dataframe(df_show[cols_s],hide_index=True,use_container_width=True)

    # ════════════════════════════════════════════════════
    # ENGENHARIA
    # ════════════════════════════════════════════════════
    elif nome_aba == "Engenharia":
        with aba_objeto:
            section_header("Painel de Engenharia", f"Hoje: {HOJE_PROJETO.strftime('%d/%m/%Y')} · Obra: {obra_selecionada or 'Nenhuma'}")
            df_eng=carregar_macro()
            df_eng=df_eng[df_eng['Obra']==obra_selecionada] if obra_selecionada else pd.DataFrame()

            def classificar_eng(dias_rest, status_tec):
                if status_tec=="Projetos Liberados para o PCP": return "concluido","Liberado para o PCP",None
                if dias_rest is None: return "sem_prazo","Aguardando programacao",None
                if dias_rest<0:  return "vencido",f"VENCIDO ha {abs(int(dias_rest))} dias",abs(int(dias_rest))
                if dias_rest<=7: return "critico",f"Critico — {int(dias_rest)} dias",int(dias_rest)
            ...
            if not df_eng.empty:
                for _,row in df_eng.iterrows():
                    ...
                    sk,st_txt,dias_num=classificar_eng(dias_rest,row.get('Status_Engenharia',ESTADOS_ENG[0]))
                    frentes.append({"id":row['id'],"edt":row['EDT'],"tarefa":row['Tarefa'],
                        ...
                        "m2":row.get('M2_Total_Tarefa',0.0),"prazo_eng":prazo_eng,"dias_restantes":dias_rest,
                        "situacao_key":sk,"situacao_txt":st_txt,"dias_num":dias_num,
                        "status_tecnico":row.get('Status_Engenharia',ESTADOS_ENG[0])})
            ...
            with st.expander(f"Frentes Criticas — {len(criticas)} alerta(s)",expanded=True):
                if not criticas: st.success("Tudo dentro do prazo!")
                else:
                    for fr in sorted(criticas,key=lambda x:x['dias_restantes'] or 0):
                        with st.container(border=True):
                            ...
                            with ci:
                                ...
                                st.markdown(eng_badge(fr['status_tecnico']),unsafe_allow_html=True)
                            with cc:
                            ...
                            with ca1:
                                idx=ESTADOS_ENG.index(fr['status_tecnico']) if fr['status_tecnico'] in ESTADOS_ENG else 0
                                ns=st.selectbox("Atualizar:",ESTADOS_ENG,index=idx,key=f"cs_{fr['id']}")
                            with ca2:
                                st.write(""); st.write("")
                                if st.button("Salvar",key=f"cb_{fr['id']}",use_container_width=True):
                                    atualizar_status_engenharia(fr['id'],ns); st.toast("Atualizado!"); time.sleep(0.3); st.rerun()

            with st.expander(f"Todas as Frentes — {len(frentes)}",expanded=False):
                if not frentes: st.info("Nenhuma frente cadastrada.")
                else:
                    cf1,cf2=st.columns([3,2])
                    with cf1: filt_st=st.selectbox("Status:",["Todos"]+ESTADOS_ENG,key="eng_fst")
                    ...
                    for fr in exibir:
                        with st.container(border=True):
                            ...
                            with ci:
                                sub=f" · *{fr['subdivisao']}*" if fr['subdivisao'] else ""
                                st.markdown(f"**{fr['tarefa']}**{sub}")
                                st.caption(f"{fr['edt']} · {fr['tipo_escopo']} · {fr['m2']:,.1f} m²")
                                st.markdown(eng_badge(fr['status_tecnico']),unsafe_allow_html=True)
                            with cd:
                                ini=pd.to_datetime(fr['inicio_previsto']).strftime('%d/%m/%Y') if prazo_valido(fr['inicio_previsto']) else "—"
                            ...
                            with cs:
                                idx=ESTADOS_ENG.index(fr['status_tecnico']) if fr['status_tecnico'] in ESTADOS_ENG else 0
                                ns=st.selectbox("",ESTADOS_ENG,index=idx,key=f"as_{fr['id']}",label_visibility="collapsed")
                            with cb:
                        ...
                        for _,sol in pend.iterrows():
                            with st.container(border=True):
                                ...
                                with sa:
                                    ...
                                    st.caption(f"*{sol['justificativa']}* · {sol['criado_por']} · {sol['criado_em']}")
                                with sb:
                                    cap,cre=st.columns(2)
                                    with cap:
                                        if st.button("Aprovar",key=f"ap_{sol['id']}",use_container_width=True): atualizar_status_solicitacao(sol['id'],"Aprovado"); st.rerun()
                                    with cre:
                                        if st.button("Rejeitar",key=f"rj_{sol['id']}",use_container_width=True): atualizar_status_solicitacao(sol['id'],"Rejeitado"); st.rerun()
                if not df_sols.empty:
                    hist=df_sols[df_sols['status']!='Pendente de Aprovacao']
                    if not hist.empty:
                        st.markdown("**Historico**")
                        for _,sol in hist.iterrows(): st.caption(f"{sol['status']} · **{sol['tarefa']}** · {sol['prazo_solicitado']} · {sol['criado_por']}")
    ...
    # ════════════════════════════════════════════════════
        ...
        with aba_objeto:
            section_header("Logistica", f"Gestao de despachos · Hoje: {HOJE_PROJETO.strftime('%d/%m/%Y')}")
            ...
            if not df_log.empty:
                ...
                st.markdown('<div class="bar-neutral">Nenhum lote na fila. Quando a producao concluir um lote, aparece aqui automaticamente.</div>',unsafe_allow_html=True)
                ...
                if df_log.empty or df_log[df_log['Status_Logistica']=='Aguardando Agendamento'].empty:
                    ...
                    for _,row in df_ag.iterrows():
                        ...
                        if prazo_valido(prazo_d):
                            ...
                            if dias_r<0:    css_bar="bar-danger"; tag=f"ATRASADO — {abs(dias_r)} dias"
                            elif dias_r<=3: css_bar="bar-warn";   tag=f"URGENTE — {dias_r} dias"
                            else:           css_bar="bar-ok";     tag=f"{dias_r} dias ate o prazo"
                        else: css_bar="bar-neutral"; tag="Sem prazo definido"
                        ...
                        with ci:
                            ...
                            st.caption(f"Pav: {row['Romaneio_Chapas']} · OP: {row.get('Num_OP') or 'S/OP'}")
                        with cp:
                            ptxt=pd.to_datetime(prazo_d).strftime('%d/%m/%Y') if prazo_valido(prazo_d) else "—"
                            st.caption("Prazo maximo despacho")
                            st.markdown(f'<span style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:20px;color:#0F1F3D;">{ptxt}</span>',unsafe_allow_html=True)
                            st.markdown(f'<span class="badge-status-{"danger" if "ATRASADO" in tag else "warn" if "URGENTE" in tag else "ok"}">{tag}</span>',unsafe_allow_html=True)
                        with ca:
                ...
                if df_log.empty or df_log[df_log['Status_Logistica']=='Envio Agendado'].empty:
                    ...
                    for _,row in df_agend.iterrows():
                        ...
                        with cp2:
                            ...
                            st.caption("Data envio"); st.markdown(f'<span style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:20px;color:#0F1F3D;">{et}</span>',unsafe_allow_html=True)
    ...
    # ════════════════════════════════════════════════════
        ...
        with aba_objeto:
            section_header("Configuracoes do Sistema", "Gestao de usuarios e reset de dados")
            ...
            conn=conectar_banco()
            df_u=pd.read_sql_query("SELECT id,usuario,nome,setor FROM usuarios ORDER BY id",conn); conn.close()
            ...
            if len(df_u)>1:
                ...
                if del_u=='master': st.caption("Conta master nao pode ser removida.")
                    ...
                    if st.button(f"Excluir {del_u}"):
                        ...
                        conn.commit(); conn.close(); st.toast("Removido!"); time.sleep(0.5); st.rerun()
            st.markdown("---")
            st.markdown('<div style="font-family:Barlow Condensed,sans-serif;font-weight:700;font-size:18px;color:#0F1F3D;margin-bottom:6px;">RESET GERAL</div>',unsafe_allow_html=True)
            st.warning("Remove TODOS os dados permanentemente.")
            if st.button("Confirmar limpeza total"):
                resetar_banco_dados_completo(); st.toast("Resetado!"); time.sleep(0.5); st.rerun()