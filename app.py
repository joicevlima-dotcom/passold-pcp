...
if st.session_state.autenticado and verificar_timeout_sessao():
 ...
 st.session_state.usuario_setor = ""
 st.session_state.ultima_atividade = None
 st.warning(f"⏱ Sessão encerrada por inatividade ({TIMEOUT_SESSAO_HORAS}h). Faça login novamente.")
...
if not st.session_state.autenticado:
 ...
 st.warning(f" {restantes} tentativa(s) restante(s) antes do bloqueio.")
...
GRUPOS_NAV = {
 ...
 "Visao Macro": (" Visão Macro", ["Master","Diretoria","Medicao"]),
 "Cadastrar Obra": (" Cadastrar Obra", ["Master"]),
 ...
 " Sistema": {
 "Configuracoes": (" Configurações", ["Master"]),
 },
}
...
with st.sidebar:
 ...
 if timeout_em:
 st.caption(f"⏱ Sessão até {timeout_em.strftime('%H:%M')}")
...
_ICONES_ACAO = {
 ...
 "EXCLUIR_FRENTE": "", "EXCLUIR_OBRA": "",
 "CRIAR_USUARIO": "", "OP_AVULSA": "",
 "SOLICITAR_OP": "", "VINCULAR_SOLICITACAO_OP": "",
 "GERAR_LOTE": "", "EXCLUIR_LOTE": "",
 "CADASTRAR_OBRA": "", "CADASTRAR_FRENTE": "",
...
if nome_aba == "Dashboard":
 ...
 if ops_pendentes > 0:
 alertas.append(("warn", "", f"{ops_pendentes} OP(s) aguardando liberação", "Acesse Liberar OPs da Semana"))
...
for nome_aba, aba_objeto in [(st.session_state.pagina_atual, _FakePage())]:
 ...
 st.warning(" Nenhuma peça lançada neste lote. Lance as peças na Seção 2 antes de concluir.")
 else:
 ...
 <span style='color:#DC2626;font-weight:700;font-size:15px;'> {len(urgentes)} lote(s) crítico(s) ou vencido(s)</span>
 </div>
 """, unsafe_allow_html=True)
 ...
 _, _, bytes_arq = conteudo_arq
 st.download_button(
 f"⬇ {arq_nome}", data=bytes(bytes_arq),
 file_name=arq_nome, mime=arq_tipo or "application/octet-stream",
 ...
 st.markdown(f"""
 <div style='background:#FEF2F2;border:1px solid #FECACA;border-radius:8px;padding:10px 18px;margin-bottom:16px;'>
 <span style='color:#DC2626;font-weight:700;font-size:15px;'> {len(urgentes_esq)} lote(s) crítico(s) ou vencido(s)</span>
 </div>""", unsafe_allow_html=True)
 ...
 with st.expander(f" Lançar Peças da OP · {n_lib_card} OP(s) liberada(s)", expanded=False):
 ...
 if st.button(" Gerar OP", key="btn_gerar_op", type="primary"):
 ...
 with st.expander(
 " Editar / Substituir Peças" if not df_pecas_existentes.empty else " Lançar Peças",
 ...
 tab_rom, tab_manual = st.tabs([" Colar Romaneio", " Manual"])
 ...
 if st.button(" Remover", key=f"del_arq_{arq_id}"):
 ...
 st.caption(" Salvar substitui a lista anterior desta OP.")
 aba_manual, aba_romaneio = st.tabs([" Manual", " Colar Romaneio"])
 ...
 with aba_objeto:
 st.markdown('<div class="page-header"><div class="page-header-left"><h2>Visão Macro</h2><p>Indicadores executivos e visão consolidada das obras</p></div><span class="page-icon"></span></div>', unsafe_allow_html=True)
 ...
 with aba_objeto:
 st.markdown('<div class="page-header"><div class="page-header-left"><h2>Cadastrar Obra</h2><p>Registre novas obras e frentes no cronograma macro</p></div><span class="page-icon"></span></div>', unsafe_allow_html=True)
 for k, v in [
 ...
 if st.button(f" Excluir frente {edt_del}", key="btn_del_frente", type="secondary"):
 st.session_state["confirm_del_frente"] = edt_del
 if st.session_state.get("confirm_del_frente") == edt_del:
 st.error(f" Confirma a exclusão da frente **{edt_del}**? Esta ação não pode ser desfeita.")
 ...
 if st.button(f" Excluir obra '{obra_del}'", key="btn_del_obra", type="secondary"):
 st.session_state["confirm_del_obra"] = obra_del
 if st.session_state.get("confirm_del_obra") == obra_del:
 st.error(f" Confirma a exclusão da obra **{obra_del}** e suas {n_frentes} frente(s)? Esta ação **não pode ser desfeita**.")
 ...
 st.markdown('<div class="empty-state"><div class="empty-icon"></div><h4>Nenhuma obra cadastrada</h4><p>Cadastre uma obra na seção acima.</p></div>', unsafe_allow_html=True)
 ...
 st.error(f" Itens em falta: {', '.join(itens_falt)}")
 elif n_conf == n_total:
 ...
 with col_excluir:
 if st.button(f" Excluir Serviço {si+1}", key=f"excluir_srv_{si}", type="secondary"):
 ...
 st.stop()
 st.markdown('<div class="page-header"><div class="page-header-left"><h2>Configurações</h2><p>Painel de controle e administração do sistema</p></div><span class="page-icon"></span></div>', unsafe_allow_html=True)
 ...
 if st.button(" Executar limpeza total", type="primary"):
