...
/* ── Widgets nativos que so tinham o tema padrao do Streamlit (select, data,
radio/checkbox/toggle fora da sidebar, upload, menus suspensos) -- sem isso eles nao
seguiam a paleta clara/escura acima. ─────────────────────────────────────────── */
.stSelectbox > div > div,
...
[data-baseweb="select"] > div { background: var(--bg-card) !important; color: var(--text) !important; }
...
[data-baseweb="popover"] li:hover { background: var(--bg) !important; }
[data-baseweb="calendar"] { background: var(--bg-card) !important; color: var(--text) !important; }
...
@media (max-width: 768px) {
    ...
    nativo do Streamlit -- nas versoes recentes (1.5x) o testid dele e'
    "stExpandSidebarButton", nao "collapsedControl" (nome de versao antiga,
    mantido abaixo so por seguranca). O header inteiro fica com
    visibility:hidden global, e visibility e' herdado -- por isso precisa
...
# ── Carrossel de fotos das obras (Dashboard) ────────
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS fotos_dashboard (
                id SERIAL PRIMARY KEY,
                nome_arquivo TEXT NOT NULL,
                tipo_arquivo TEXT,
                conteudo BYTEA NOT NULL,
...
@st.cache_data(ttl=30)
def carregar_fotos_dashboard(limite: int = 24):
    """Carrossel do Dashboard -- ja vem redimensionada/comprimida no upload
    (_preparar_foto_dashboard), entao trazer o conteudo de todas de uma vez so
...
def salvar_foto_dashboard(nome: str, tipo: str, conteudo: bytes, legenda: str, usuario: str) -> bool:
    conn = conectar_banco()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO fotos_dashboard (nome_arquivo, tipo_arquivo, conteudo, legenda, enviado_por) VALUES (%s,%s,%s,%s,%s)",
            (nome, tipo, conteudo, legenda, usuario)
        )
        conn.commit()
        carregar_fotos_dashboard.clear()
        return True
    except Exception as e:
        conn.rollback()
...
def deletar_foto_dashboard(foto_id: int) -> bool:
    conn = conectar_banco()
    try:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM fotos_dashboard WHERE id=%s", (foto_id,))
        conn.commit()
        carregar_fotos_dashboard.clear()
        return True
    except Exception as e:
...
def _preparar_foto_dashboard(nome: str, tipo: str, conteudo: bytes) -> tuple[str, str, bytes]:
    """Redimensiona e recomprime qualquer foto pro carrossel do Dashboard. Essa tela e'
    vista por todo mundo o tempo todo (auto-refresh nao, mas e' a home de todo login) --
    guardar a foto original do celular em alta (varios MB) pesaria em toda visita, entao
    aqui SEMPRE reduz (max 1000px no maior lado, JPEG qualidade 80), diferente dos outros
    uploads do sistema que preservam o arquivo original. Aceita HEIC tambem (mesma
    deteccao por assinatura do _eh_conteudo_heic, nao so pela extensao)."""
    try:
        import io
        if _eh_conteudo_heic(conteudo):
            import pillow_heif
            pillow_heif.register_heif_opener()
        from PIL import Image, ImageOps
        img = Image.open(io.BytesIO(conteudo))
        img = ImageOps.exif_transpose(img)
        img.thumbnail((1000, 1000))  # mantem proporcao, so' reduz (nunca aumenta foto pequena)
        out = io.BytesIO()
        img.convert("RGB").save(out, format="JPEG", quality=80, optimize=True)
        novo_nome = (nome.rsplit('.', 1)[0] if '.' in nome else nome) + '.jpg'
        return novo_nome, "image/jpeg", out.getvalue()
    except Exception:
        return nome, tipo, conteudo  # se a compressao falhar por qualquer motivo, guarda o original em vez de travar o upload
...
def atualizar_dados_item_insumo(item_id: int, descricao: str, quantidade: float, unidade: str):
    """Corrige descricao/quantidade/unidade de um item de saida de insumos lancado errado."""
    conn = conectar_banco()
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE saidas_insumos_itens SET descricao=%s, quantidade=%s, unidade=%s WHERE id=%s",
            (descricao, quantidade, unidade, item_id)
        )
        conn.commit()
        carregar_itens_saida_insumos.clear()
        carregar_todos_itens_insumos.clear()
...
with tb_esq:
    st.markdown('<span style="font-size:1.1rem;font-weight:700;color:var(--primary);">Passold <span style="color:var(--accent);">Sistemas de Fachadas</span></span>', unsafe_allow_html=True)
        ...
        with aba_objeto:
            ...
            with st.expander("➕ Liberação de OP  ·  Ancoragens, prisílias e materiais de apoio", expanded=True):
                ...
                if df_projetos.empty:
                    ...
                    with av3:
                        ...
                        # de apoio (ancoragem, prisilia, corte de perfil) pode ser produzido por um
                        # time diferente do escopo principal da obra.
            ...
            with st.expander(f"✏️ Lançar Peças da OP  ·  {n_lib_card} OP(s) liberada(s)", expanded=False, key="op_lancar_pecas_expander"):
                ...
                incluir_concluidas_fat = st.toggle(
                    ...
                    value=False, key="fat_incl_concluidas"
                )
                st.markdown("---")
                ...
                status_fat = ["Liberado para Fabrica", "Parcialmente Concluido"]
                if incluir_concluidas_fat:
                    status_fat.append("Concluido")

                if not df_banco_micro.empty:
                    mask_fat = df_banco_micro['Status_Item'].isin(status_fat)
        ...
        with aba_objeto:
            ...
            with st.container():
                ...
                if df_conc_log.empty:
                    ...
                    def _render_ops_por_data(df_slice):
                        """Renderiza as OPs de df_slice separadas por sub-cabecalho de data
                        (Hoje / Ontem / ...), mais recente primeiro. Sem expander -- serve
                        pra usar dentro do expander de obra (nada de expander aninhado)."""
                        for ord_sb in sorted(df_slice['_bk_ord'].unique()):
                            g_sb = df_slice[df_slice['_bk_ord'] == ord_sb].sort_values('_data_pronto', ascending=False)
                            n_parc_sb = int((g_sb['Status_Item'] == 'Parcialmente Concluido').sum())
                            cab_sb = f"{g_sb.iloc[0]['_bk_label']} · {len(g_sb)} OP(s)"
                            if n_parc_sb:
                                cab_sb += f" · 🟠 {n_parc_sb} parcial(is)"
                            st.markdown(f"###### {cab_sb}")
                            for _, row_c in g_sb.iterrows():
                                _render_op_pronta(row_c)

                    modo_org_op = st.radio(
                        "Organizar por:", ["🏗️ Obra (com data)", "📅 Só por data"],
                        horizontal=True, key="log_op_org",
                        help="\"Obra (com data)\" separa por obra e, dentro de cada obra, por data que ficou pronto. "
                             "\"Só por data\" junta todas as obras e separa só pela data. "
                             "A data é o dia em que a produção finalizou a OP (ou a do último envio, no caso de OP parcial)."
                    )
                    ...
                    if modo_org_op.startswith("📅"):
                        for ord_b in sorted(df_conc_log['_bk_ord'].unique()):
                            ...
                            titulo_bk = f"{grp_bk.iloc[0]['_bk_label']} — {len(grp_bk)} OP(s)"
                            if n_parc_bk:
                        ...
                        _render_ops_por_data(df_conc_log)
                    else:
                        ...
                        for ob_row in resumo_op.itertuples():
                            ...
                            with st.expander(titulo_op, expanded=False, key=f"log_op_pronta_expander_{ob_row.Obra_Vinculada}"):
                                ...
                                _render_ops_por_data(df_obra_op)
    ...
    if nome_aba == "Relatorio Geral":
        with aba_objeto:
            ...
            with st.expander("📅 Relatório Semanal — ACM e Esquadrias em abas separadas", expanded=True, key="rel_semanal_expander"):
                ...
                with rs1:
                    obras_rs = ["Todas as Obras"] + (
                        sorted(df_banco_micro_rel['Obra_Vinculada'].dropna().unique().tolist())
                        if not df_banco_micro_rel.empty else []
                    )
                    rs_obra = st.selectbox("Obra:", obras_rs, key="rs_obra")
                with rs2:
                    rs_dt_ini = st.date_input(
                        "Concluído de:", value=(hoje_projeto() - timedelta(days=7)).date(),
                        format="DD/MM/YYYY", key="rs_dt_ini"
                    )
                with rs3:
                    rs_dt_fim = st.date_input(
                        "Concluído até:", value=hoje_projeto().date(),
                        format="DD/MM/YYYY", key="rs_dt_fim"
                    )

                def _fmt_data_rs(serie):
                ...
                def _preparar_bloco_semanal(df_secao, escopo_valor, apenas_concluidos, df_saldo_pecas, equipe_terc=None):
                    ...
                    col_qtd_label = 'PEÇAS' if eh_kg else 'CAIXAS'

                    def _total_row(row):
                        base = row.get(col_real)
                        return float(base) if pd.notna(base) and base and base > 0 else float(_nn(row.get(col_base), 0))

                    def _restante_row(row, total):
                        if apenas_concluidos or row['Status_Item'] == 'Concluido':
                            return 0.0
                        qtd_total_p = row.get('qtd_total')
                        if pd.notna(qtd_total_p) and qtd_total_p > 0:
                            return total * (float(_nn(row.get('saldo'), 0)) / float(qtd_total_p))
                        return total

                    df_e['_total'] = df_e.apply(_total_row, axis=1)
                    df_e['_restante'] = df_e.apply(lambda r: _restante_row(r, r['_total']), axis=1)
                    df_e['_executado'] = df_e['_total'] - df_e['_restante']
                    df_e['_pct'] = df_e.apply(
                        lambda r: round(r['_executado'] / r['_total'] * 100) if r['_total'] > 0 else 100, axis=1
                    )
                    if 'Em_Parada' in df_e.columns:
                        df_e['_situacao'] = df_e.apply(
                            lambda r: f"⛔ PARADA — {_nn(r.get('Motivo_Parada'), '')}" if r.get('Em_Parada') else '', axis=1
                        )
                    else:
                        df_e['_situacao'] = ''

                    out = pd.DataFrame({
                        'OBRA': df_e['Obra_Vinculada'],
                        'OP': df_e['Num_OP'],
                        'MATERIAL': df_e['Tipo_Material'],
                        'EDT/LOTE': df_e['EDT_Vinculado'],
                        col_qtd_label: df_e['Qtd_Caixas'],
                        f'TOTAL {label_medida}': df_e['_total'].round(2),
                        f'{label_medida} EXECUTADO': df_e['_executado'].round(2),
                        f'{label_medida} RESTANTE': df_e['_restante'].round(2),
                        'INI PROD.': _fmt_data_rs(df_e['Data_Producao_Programada']),
                        'LIMITE': _fmt_data_rs(df_e['Data_Limite_Obra']),
                        'CONCLUSÃO': _fmt_data_rs(df_e['Concluido_Em']) if 'Concluido_Em' in df_e.columns else '—',
                        'STATUS': df_e['Status_Item'],
                        'SITUAÇÃO': df_e['_situacao'],
                        'CONCLUIDO': df_e['_pct'].astype(int).astype(str) + '%',
                    })
                ...
                if st.button("📊 Gerar Relatório Semanal", key="btn_gerar_rel_semanal", type="primary"):
                    if rs_dt_fim < rs_dt_ini:
                        st.error("A data 'Até' não pode ser anterior à data 'De'.")
                    else:
                        df_base_rs = df_banco_micro_rel.copy()
                        if rs_obra != "Todas as Obras" and not df_base_rs.empty:
                            df_base_rs = df_base_rs[df_base_rs['Obra_Vinculada'] == rs_obra]
                        df_saldo_rs = carregar_saldo_pecas_por_lote()

                        df_concluido_rs = df_base_rs[df_base_rs['Status_Item'] == 'Concluido'].copy() if not df_base_rs.empty else pd.DataFrame()
                        if not df_concluido_rs.empty:
                            dt_concl_rs = pd.to_datetime(df_concluido_rs['Concluido_Em'], errors='coerce')
                            df_concluido_rs = df_concluido_rs[
                                (dt_concl_rs >= pd.Timestamp(rs_dt_ini)) &
                                (dt_concl_rs < pd.Timestamp(rs_dt_fim) + timedelta(days=1))
                            ]
                ...
                if st.session_state.get('_rel_semanal_bytes'):
                    for titulo_res, n_res in st.session_state.get('_rel_semanal_resumo', []):
                        st.caption(f"✓ {titulo_res}: {n_res} item(ns)")
                    st.download_button(
                        "⬇️ Baixar Relatório Semanal",
                        data=st.session_state['_rel_semanal_bytes'],
                        file_name=f"relatorio_semanal_{datetime.now(FUSO_BR).strftime('%Y%m%d_%H%M')}.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
